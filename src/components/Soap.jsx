...
function Soap() {
  ...
  return (
    <>
      ...
      {soap.map((item,key)=>(
        <ProCard className={`flex-shrink-0 w-fit ${cartItem.find(i=>i.image==item.image)?'border-2 border-green-500':''} `} key={key} image={item.image} title={item.title} price={item.price}/>
      ))}
      </div>
